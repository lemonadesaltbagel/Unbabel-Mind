...
export const updateUser=async(i:number,f:string,l:string,e:string):Promise<User>=>{const q='UPDATE users SET first_name = $1, last_name = $2, email = $3 WHERE id = $4 RETURNING *';const r=await pool.query(q,[f,l,e,i]);return r.rows[0];};
