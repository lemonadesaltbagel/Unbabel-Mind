...
return(<div className="min-h-screen bg-black text-black p-6 flex flex-col items-center"><div className="w-full max-w-full flex justify-between items-center mb-4"><button onClick={()=>r.push('/dashboard')}><Home className="w-6 h-6 text-white hover:text-blue-500 transition"/></button><div className="text-white text-xl font-mono">{tm}</div><div></div></div><div className="flex flex-col lg:flex-row space-y-6 lg:space-y-0 lg:space-x-4 w-full max-w-full"><ReadingPassage title={pt} content={pc} highlights={highlights} onContextMenu={handleContextMenu}/><QuestionList questions={qs} answers={a} setAnswers={ha}/></div><ReadingControls questionType={qt} isSubmitting={is} onSubmit={hsub} onNavigate={hn}/></div>);}
