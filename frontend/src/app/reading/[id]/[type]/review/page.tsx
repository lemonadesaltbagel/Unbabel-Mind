...
Respond in JSON format only:
{
  ...
  const he = async (qn: number, ua: string, ca: string) => {
    ...
    try {
      ...
      const prompt = `You are an expert IELTS tutor with deep knowledge of reading comprehension strategies.\n\nQuestion Type: ${q.type}\nQuestion: ${q.question}\nEvidence: ${qEvidence.map(e => e.text).join('\n')}\nAvailable Options: ${q.options?.join(', ') || 'N/A'}\nUser Answer: ${ua}\nCorrect Answer: ${ca}\n\nPlease explain why the correct answer is \"${ca}\", analyze why the user chose \"${ua}\", and provide guidance.`;
      const response = await callGptApi(prompt);
      setAiResponse(response);
    } finally {
      setIsLoading(false);
