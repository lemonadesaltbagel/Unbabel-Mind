...
export default function SpeakingPage() {
  ...
  const lk = `speaking-answers-${id}-${type}`;
  const [pt, setPt] = useState('');
  const [pc, setPc] = useState('Loading speaking prompt...');
  ...
  useEffect(() => {
    const lp = async () => {
      try {
        const res = await fetch(`/static/speaking/${id}_${type}.txt`);
        ...
        setPt('');
        setPc('Failed to load speaking prompt.');
      }
    };
    lp();
  }, [id, type]);
  useEffect(() => {
    const lq = async () => {
      try {
        const res = await fetch(`/static/speaking/${id}_${type}_q.json`);
    ...
    <div className="min-h-screen bg-black text-black p-4 sm:p-6 flex flex-col items-center">
      ...
      <div className="flex flex-col lg:flex-row space-y-6 lg:space-y-0 lg:space-x-6 w-full max-w-6xl">
        <div className="bg-white p-6 rounded-xl shadow w-full lg:w-1/2 h-[80vh] overflow-y-auto">
          <h2 className="text-xl font-bold mb-2">Speaking Section {type}</h2>
