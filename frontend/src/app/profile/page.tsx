    ...
    <div className="min-h-screen bg-black text-white flex items-center justify-center">
      <div className="bg-gray-900 p-8 rounded-xl shadow-xl w-full max-w-md text-center">
        ...
        <div className="flex justify-center space-x-4 mt-6">
          {editing ? (
            <>
              <button onClick={handleSave} className="bg-green-600 hover:bg-green-700 px-4 py-2 rounded">
                Save
              </button>
              <button onClick={() => setEditing(false)} className="bg-gray-600 hover:bg-gray-700 px-4 py-2 rounded">
                Cancel
              </button>
            </>
          ) : (
            <button onClick={() => setEditing(true)} className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded">
              Edit Profile
            </button>
          )}
        </div>
        ...
        <div className="mt-8 border-t border-gray-700 pt-6">
          <h2 className="text-xl font-semibold mb-4">LLM Token Configuration</h2>
          <div className="space-y-4 text-left">
            <div>
              <label className="block text-gray-400">OpenAI API Token:</label>
              {tokenEditing ? (
                <>
                  <input
                    type="text"
                  ...
                  />
                  {tokenError && (
                    <p className="mt-1 text-red-400 text-sm">{tokenError}</p>
                  )}
                </>
              ) : (
                <p className="mt-1 text-sm text-gray-300">
                  {openaiToken ? blurToken(openaiToken) : 'Not configured'}
                </p>
              )}
              ...
              <div className="mt-2">
                {tokenEditing ? (
                  <div className="flex space-x-2">
                    <button onClick={handleTokenSave} className="bg-green-600 hover:bg-green-700 px-3 py-1 rounded text-sm">
                      Save
                    </button>
                    <button 
                      onClick={() => {
                  ...
                  <button 
                    onClick={() => {
                      ...
                      setEditingToken(openaiToken);
                    }} 
                    className="bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded text-sm"
                  >
                    {openaiToken ? 'Edit' : 'Add'} Token
                  </button>
                )}
              </div>
              
              {tokenSuccess && (
                <p className="mt-2 text-green-400 text-sm">Token saved successfully!</p>
              )}
            </div>
          </div>
        </div>
