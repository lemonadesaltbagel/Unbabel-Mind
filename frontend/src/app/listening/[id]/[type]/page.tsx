...
useTestPageTitle();
const r=useRouter();const p=useParams();const{user,loading}=useAuth();const{id,type}=p as{id:string;type:string};const pid=Number(id);const qt=Number(type);const lk=`listening-answers-${id}-${type}`;const[pt,setPt]=useState('');const[pc,setPc]=useState('Loading audio transcript...');const[qs,setQs]=useState<Question[]>([]);const[a,setA]=useState<Record<number,string[]>>({});const[is,setIs]=useState(false);const il=useRef(true);
